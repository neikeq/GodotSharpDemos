...
{
    string current_anim = string.Empty;
    ...
    const float WALK_ACCEL = 800.0f;
    const float WALK_DEACCEL = 800.0f;
    const float WALK_MAX_VELOCITY = 200.0f;
    const float AIR_ACCEL = 200.0f;
    const float AIR_DEACCEL = 200.0f;
    const float STOP_JUMP_FORCE = 900.0f;
    const float MAX_FLOOR_AIRBORNE_TIME = 0.15f;
    const float MAX_SHOOT_POSE_TIME = 0.3f;

    const int JUMP_VELOCITY = 460;
    ...
    void _integrate_forces(Physics2DDirectBodyState s)
    {
        ...
        // Get the controls
        bool move_left = Input.is_action_pressed("move_left");
        bool move_right = Input.is_action_pressed("move_right");
        bool jump = Input.is_action_pressed("jump");
        bool shoot = Input.is_action_pressed("shoot");
        bool spawn = Input.is_action_pressed("spawn");
		...
		*/
        if (shoot && !shooting)
        {
            ...
            Physics2DServer.body_add_collision_exception(bi.get_rid(), get_rid()); // Make bullet and this not collide
        }
        else
        {
            shoot_time += step;
